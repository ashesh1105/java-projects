...
public class RemoveRedundantBrackets {
    ...
    private static String removeParenthesis(StringBuilder sb) {
    ...
    private static boolean removeParenthesis(Integer leftPrecedence, StringBuilder sb, int start) {
        ...
        while (start < sb.length()) {
            if (sb.charAt(start) == '(') {
                ...
                Integer rightPrecedence = start == sb.length() - 1 || sb.charAt(start + 1) == ')' ? null: getPrecedence(sb.charAt(start + 1));
                if ((leftPrecedence != null && minPrecedence < leftPrecedence)|| (rightPrecedence != null && minPrecedence < rightPrecedence)) {
                    return false;
                } else {
                    sb.deleteCharAt(start);
                    return true;
