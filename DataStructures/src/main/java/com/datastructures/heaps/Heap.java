...
public class Heap {
		...
		while (index <= upto) {
			...
			if (leftChild <= upto) {
				...
				if (rightChild > upto)
					...
					childToSwap = (heapData[leftChild] > heapData[rightChild]) ? leftChild
							: rightChild;
	...
	public void heapSort() {
		for (int i = 0; i < currentPosition; i++) {
			...
			heapData[0] = heapData[currentPosition - i]; // bring last element
															// to the root
			heapData[currentPosition - i] = tmp; // put max at the last of
													// unsorted part
