package com.algorithms.hackerrank.tree.dfs;
...
/*
A demonstration of Open-Close Principle with Tree. Run this program with following input:
- First line has integer n for number of nodes in tree
- Second line has values for nodes, beginning from 1st node. Third line has color of nodes, 0 for
Color.RED and 1 for Color.GREEN
- Next n-1 lines indicate the edges between node u and v. It always starts with node 1

1. Creates a SumInLeavesVisitor object whose getResult method returns the sum of the leaves in the tree, which is
7+5+12 = 24. Code prints the returned value on a new line.
2. Creates a ProductOfRedNodesVisitor object whose getResult method returns the product of the red nodes, which is
4*2*5 = 40. Code prints the returned value on a new line.
3. Creates a FancyVisitor object whose getResult method returns the absolute difference between the sum of the
values of non-leaf nodes at even depth and the sum of the values of green leaf nodes, which is
4-(7+12) = 15. The locked stub code prints the returned value on a new line.
